...
t2 = Traffic.from_file("t2_0722_noonground2.parquet")
# t2 = t2[:1000]
# assert t2 is not None
# t2 = t2.query(f"altitude>{altitude_min}")
# assert t2 is not None
# t2 = t2.clip(aixm_airspaces[extent]).filter().resample("1s").eval(max_workers=8)
assert t2 is not None
...
print(nt2)
# print(sum([len(sub) for sub in subsetst2]))

# POUR LE BUG
couples_datas = [
    (sub, f"stats_devs_pack/stats_para_{i}.parquet")
    for i, sub in enumerate(subsetst2)
]
# couples_datas = [
#     (
#         t2[nbsubsets * (nbworkers - 1) :],
#         f"stats_devs_pack/stats_para_{nbworkers-1}.parquet",
#     )
# ]
        ...
        try:
            ...
            for flight_trou in flight - flight.aligned_on_navpoint(
                ...
                if (
                    ...
                    # we only want the stable portion of flight_interest :
                    # flight_interest = flight_interest.query(
                    #     f"{flmin}<=altitude<={flmax}"
                    # )
                    # assert flight_interest is not None
