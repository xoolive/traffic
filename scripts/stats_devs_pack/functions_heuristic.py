...
def predict_fp(
    ...
    section = cast(Flight, flight.before(trou.start, strict=False)).last(
        minutes=20
    )
    ...
    data_points["latitude"].append(
        cast(
            Position, cast(Flight, flight.before(trou.start)).at_ratio(1)
        ).latitude
    )
    ...
    new_columns = {
        **data_points,
        ...
        "altitude": cast(Position, flight.at(trou.start)).altitude,
