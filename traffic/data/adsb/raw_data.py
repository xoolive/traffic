...
class RawData(pd.DataFrame):
    ...
    @staticmethod
    def position_extraction(os_pos,
    ...
    def feature_extraction(os_data,
        ...
        vel = RawData.velocity_extraction(
            os_data.loc[os_data['msg_type'] == 4])
    ...
    def get_type(self, inplace=True):
        ...
        if inplace:
            self['msg_type'] = self.rawmsg.apply(get_typecode)
            self.dropna(subset=["msg_type"], inplace=True)
        else:
            return self.rawmsg.apply(get_typecode)

    def to_beast(self) -> pd.Series:
        df_beast = self[['timestamp', 'rawmsg']].copy()
        ref_time = self['timestamp'].iloc[0]
        df_beast['time'] = self['timestamp'] - ref_time
