...
class NavaidParser(object):
    ...
    def __getitem__(self, name: str) -> Optional[Navaid]:
        return next((pt for pt in self.navaids
                     if (pt.id == name.upper())), None)

    def search(self, name: str) -> List[Navaid]:
        return list((pt for pt in self.navaids
                     if (pt.description is not None and
                             (re.match(name, pt.description, re.IGNORECASE)))
                     or (pt.id == name.upper())))
    ...
    def initialize(self):
        ...
        for line in c.iter_lines():
            ...
            fields = line.split()
            self.navaids.append(Navaid(
                fields[2], 'FIX', float(fields[0]), float(fields[1]),
                None, None, None, None))
    ...
    #   0      1         2              3    4   5      6   7     8
            ...
            # Type names
            wptypedict = {2:"NDB", 3:"VOR", 4:"ILS", 5:"LOC", 6:"GS",
                          7:"OM", 8:"MM", 9:"IM", 12:"DME", 13:"TACAN"}
           ...
           # Type code never larger than 20
            ...
            self.navaids.append(Navaid(
                fields[7], wptype, float(fields[1]), float(fields[2]),
                float(fields[3]),
                float(fields[4]) if wptype=='NDB' else float(fields[4])/100,
